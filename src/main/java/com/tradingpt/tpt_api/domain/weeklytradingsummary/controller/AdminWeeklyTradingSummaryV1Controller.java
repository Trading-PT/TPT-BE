...
public class AdminWeeklyTradingSummaryV1Controller {
	...
	@Deprecated
	@Operation(
		summary = "[Deprecated] 주간 매매 일지 통계 작성 (Trainer)",
		description = """
			⚠️ **Deprecated**: PUT /customers/{customerId}/years/{year}/months/{month}/weeks/{week}/evaluation API를 사용하세요.
	...
	@Operation(
		summary = "주간 매매일지 트레이너 평가 Upsert (Trainer)",
		description = """
			트레이너가 고객의 주간 매매일지 평가를 생성하거나 수정합니다. (Upsert 패턴)

			⭐ 작성 규칙:

			✅ 완강 후 (AFTER_COMPLETION) + DAY 타입:
			   - 트레이너 평가 작성/수정 가능
			   - weeklyEvaluation: 매매 최종 평가 및 개선점
			   - weeklyProfitableTradingAnalysis: 수익난 매매 분석
			   - weeklyLossTradingAnalysis: 손실난 매매 분석

			❌ 완강 전 (BEFORE_COMPLETION, PENDING_COMPLETION):
			   - 트레이너는 평가 불가 (고객이 메모 작성)

			❌ 완강 후 (AFTER_COMPLETION) + SWING 타입:
			   - 주간 평가 불가 (월간 평가만 가능)

			동작 방식:
			- 해당 주에 통계가 없으면 새로 생성
			- 해당 주에 통계가 있으면 평가만 수정 (JPA Dirty Checking)
			"""
	)
	@PutMapping("/customers/{customerId}/years/{year}/months/{month}/weeks/{week}/evaluation")
	@PreAuthorize("hasRole('ROLE_ADMIN') or hasRole('ROLE_TRAINER')")
	public BaseResponse<WeeklyEvaluationResponseDTO> upsertWeeklyEvaluationByTrainer(
		@Parameter(description = "고객 ID", required = true)
		@PathVariable Long customerId,
		@Parameter(description = "연도", example = "2025", required = true)
		@PathVariable Integer year,
		...
		@Parameter(hidden = true)
		@AuthenticationPrincipal(expression = "id") Long trainerId,
		@Valid @RequestBody UpsertWeeklyEvaluationRequestDTO request
	) {
		return BaseResponse.onSuccess(
			weeklyTradingSummaryCommandService.upsertWeeklyEvaluationByTrainer(
				year, month, week, customerId, trainerId, request)
		);
	}
}
