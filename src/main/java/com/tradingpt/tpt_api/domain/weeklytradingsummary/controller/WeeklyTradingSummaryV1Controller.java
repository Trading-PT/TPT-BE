...
public class WeeklyTradingSummaryV1Controller {
	...
	@Deprecated
	@Operation(
		summary = "[Deprecated] 주간 매매 일지 통계 작성 (Customer)",
		description = """
			⚠️ **Deprecated**: PUT /customers/me/years/{year}/months/{month}/weeks/{week}/memo API를 사용하세요.
	...
	@Operation(
		summary = "주간 매매일지 메모 Upsert (Customer)",
		description = """
			...
			동작 방식:
			- 해당 주에 통계가 없으면 새로 생성
			- 해당 주에 통계가 있으면 메모만 수정 (JPA Dirty Checking)
			"""
	)
	@PutMapping("/customers/me/years/{year}/months/{month}/weeks/{week}/memo")
	public BaseResponse<WeeklyMemoResponseDTO> upsertWeeklyMemoByCustomer(
		@Parameter(description = "연도", example = "2025", required = true)
		@PathVariable Integer year,
		@Parameter(description = "월 (1-12)", example = "8", required = true)
		@PathVariable Integer month,
		...
		@PathVariable Integer week,
		@AuthenticationPrincipal(expression = "id") Long customerId,
		@Valid @RequestBody UpsertWeeklyMemoRequestDTO request
	) {
		return BaseResponse.onSuccess(
			weeklyTradingSummaryCommandService.upsertWeeklyMemoByCustomer(
				year, month, week, customerId, request)
		);
	}
}
