...
public class ComplaintV1Controller {
    ...
    @PostMapping
    public CreateComplaintResponseDTO createComplaint(Authentication auth, @Valid @RequestBody CreateComplaintRequestDTO request) {
        Long userId = ((AuthSessionUser) auth.getPrincipal()).id();
        return complaintCommandService.createComplaint(userId, request);
    }

    @Operation(summary = "내 민원 목록 조회", description = "유저 ID로 자신의 민원 목록을 조회합니다.")
    @GetMapping
    public List<ComplaintResponseDTO> getUserComplaints(Authentication auth) {
        Long userId = ((AuthSessionUser) auth.getPrincipal()).id();
        return complaintQueryService.getComplaintsByUser(userId);
    }
}
