...
- **어떻게**: 로컬 프론트엔드(localhost:3000)에서 AWS 서버(dev.tradingpt.kr)로 인증된 API 호출 시 지속적인 403 Forbidden 오류 발생
- **증상**: 로그인은 성공하지만, 이후 모든 상태 변경 API 요청에서 CSRF 검증 실패

### 환경 정보

- **시스템**:
    - Backend: AWS EC2 (dev.tradingpt.kr), Spring Boot 3.3.5, Spring Security 6.x
...
```
1. 프론트엔드(localhost:3000)에서 로그인 요청 → dev.tradingpt.kr/api/v1/auth/login
...
Invalid CSRF token found for http://dev.tradingpt.kr/api/v1/memo
...
- **확장성 문제**:
    - 프로덕션 환경(tradingpt.kr)에서는 정상 작동 (Same-Origin)
...
2. **Cross-Origin 호환**: 로컬 개발 환경(localhost:3000 ↔ dev.tradingpt.kr)에서 정상 작동
 ...
 *
 * 목적:
 * - Cross-Origin 개발 환경(localhost:3000 ↔ dev.tradingpt.kr)에서
...
- ✅ **Cross-Origin 개발 환경 정상화**: localhost:3000에서 dev.tradingpt.kr API 완전 호출 가능
- ✅ **보안성 유지**: CSRF 보호를 비활성화하지 않고 표준 방식 유지
- ✅ **프론트엔드 개발 생산성 향상**: 로컬 개발 시 실시간 테스트 가능 (배포 없이)
- ✅ **코드 재사용성**: 프로덕션 환경(tradingpt.kr)에서도 동일 로직 사용, 환경별 분기 불필요
