...
- **어떻게**: 직접 테스트 - `https://dev.tradingpt.kr/oauth2/authorization/kakao` 접근
- **증상**:
  - 1차: 404 에러 (정적 리소스를 찾을 수 없음)
  - 2차: 카카오 로그인 화면은 표시되나, 로그인 완료 후 `AUTH_401_12` 에러

### 환경 정보
- **시스템**: Dev 환경 (`https://dev.tradingpt.kr`)
...
#### 재현 시나리오
```
1. 브라우저에서 https://dev.tradingpt.kr/oauth2/authorization/kakao 접근
...
#### 재현 시나리오
```
1. https://dev.tradingpt.kr/oauth2/authorization/kakao 접근
2. 카카오 로그인 페이지로 정상 리다이렉트 ✅
3. 카카오에서 로그인 완료
4. 콜백 URL로 리다이렉트: https://dev.tradingpt.kr/login/oauth2/code/kakao?code=...&state=...
...
[2] 카카오 로그인 페이지 (사용자 로그인)
                    │
                    ▼
[3] 카카오 → https://dev.tradingpt.kr/login/oauth2/code/kakao?code=...&state=...
                    │
                    ▼
┌─────────────────────────────────────────────────────────────────────────────┐
...
#### 핵심 문제
- Dev/Prod 환경은 HTTPS (`https://dev.tradingpt.kr`)
